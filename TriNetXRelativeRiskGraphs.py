...
st.caption('Tip: Rows with "##" at the start of the cohort name become group headings.')
data = st.data_editor(
    default_data,
    num_rows="dynamic",
...
custom_xlabel = st.sidebar.text_input("X Axis Title", "Relative Risk" if orientation == "Horizontal" else "Cohort")
custom_ylabel = st.sidebar.text_input("Y Axis Title", "Cohort" if orientation == "Horizontal" else "Relative Risk")
...
for i, row in data.iterrows():
    ...
    if name.startswith("##"):
        ...
        group_boundaries.append(len(plot_labels))  # Where group starts
...
if orientation == "Horizontal":
    ...
    for i, bar in enumerate(bars):
        value = bar.get_width()
        if label_pos == "Inside":
            ax.text(value / 2, bar.get_y() + bar.get_height() / 2, f"{value:.2f}", va='center', ha='center',
                    color='white' if not grayscale else 'black', fontweight="bold")
        else:
            ax.text(value + spacing/100, bar.get_y() + bar.get_height() / 2, f"{value:.2f}",
                    va='center', ha='left', color=axis_color, fontweight="bold")
    ax.invert_yaxis()
else:
    ...
    for i, bar in enumerate(bars):
        ...
        if label_pos == "Inside":
            ax.text(bar.get_x() + bar.get_width() / 2, value / 2, f"{value:.2f}", ha='center', va='center',
                    color='white' if not grayscale else 'black', fontweight="bold")
        else:
            ax.text(bar.get_x() + bar.get_width() / 2, value + spacing/100, f"{value:.2f}",
                    ha='center', va='bottom', color=axis_color, fontweight="bold")
...
for idx, group_start in enumerate(group_boundaries):
    if orientation == "Horizontal":
        ...
        if group_start < len(plot_labels):
            ax.text(ax.get_xlim()[0], group_start, group_labels[idx], va='center', ha='left',
                    color=axis_color, fontweight="bold", fontsize=font_size + 1, bbox=dict(facecolor=bg_color, edgecolor='none'))
    else:
        ax.axvline(group_start - 0.5, color=axis_color, linewidth=1, linestyle='--')
        if group_start < len(plot_labels):
            ax.text(group_start, ax.get_ylim()[1], group_labels[idx], va='bottom', ha='center',
                    color=axis_color, fontweight="bold", fontsize=font_size + 1, bbox=dict(facecolor=bg_color, edgecolor='none'))
...
# --- Options Recap ---
with st.expander("Available Options and Features"):
    st.markdown("""
...
- **Grid Lines**: Toggle gridlines on/off for cleaner or more analytic look.
